...
num_list = [wdwlessnum, bestofallnum, wdwresnum, twovisitnum, twovisitwdwresnum, twovisitwdwslresnum, twovisitslresnum, twovisit2wdwnum, twovisit2wdwslresnum, twovisit3wdwnum, twovisit3wdwslresnum]
num_save = []
for i in range(len(num_list)):
    num_save.append(num_list[i].index(min(num_list[i])))
    num_list[i] = min(num_list[i])
text_list = [wdwlesstext, bestofalltext, wdwrestext, twovisittext, twovisitwdwrestext, twovisitwdwslrestext, twovisitslrestext, twovisit2wdwtext, twovisit2wdwslrestext, twovisit3wdwtext, twovisit3wdwslrestext]
    ...
    for j in range(len(downlist[0][i][1])):
        ...
        if downlist[1][downlist[0][i][1][j]][0] == "Original":
            for k in range(0, len(text_list[num_list.index(downlist[0][i][2][j])])):
                downlist4[downlist[0][i][0]][downlist[1][downlist[0][i][1][j]][0]].append(text_list[num_list.index(downlist[0][i][2][j])][k]) # Upstairs 1

            downlist4[downlist[0][i][0]][downlist[1][downlist[0][i][1][j]][0]].append("BitS")
        else:
            for k in range(0, text_list[num_list.index(downlist[0][i][2][j])].index("Down")):
                downlist4[downlist[0][i][0]][downlist[1][downlist[0][i][1][j]][0]].append(text_list[num_list.index(downlist[0][i][2][j])][k]) # Upstairs 1

            for k in range(len(downlist[1][downlist[0][i][1][j]][3])):
                downlist4[downlist[0][i][0]][downlist[1][downlist[0][i][1][j]][0]].append(downlist[1][downlist[0][i][1][j]][3][k]) # Downstairs 2

            if downlist[1][downlist[0][i][1][j]][0] not in ["Classic", "Why", "Late VC"]:
                if "jetstream" in starTimes[downlist[0][i][4]][2][4]: downlist4[downlist[0][i][0]][downlist[1][downlist[0][i][1][j]][0]].append("JRB") # JRB 2

            for k in range(text_list[num_list.index(downlist[0][i][2][j])].index("Down")+1, len(text_list[num_list.index(downlist[0][i][2][j])])):
                downlist4[downlist[0][i][0]][downlist[1][downlist[0][i][1][j]][0]].append(text_list[num_list.index(downlist[0][i][2][j])][k]) # Upstairs 2

            downlist4[downlist[0][i][0]][downlist[1][downlist[0][i][1][j]][0]].append("BitS")
        if summed < best: 
            best = summed
    print(downlist[0][i][0], downlist3[downlist[0][i][0]], downlist4[downlist[0][i][0]])
